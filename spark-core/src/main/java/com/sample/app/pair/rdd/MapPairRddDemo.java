...
public class MapPairRddDemo {
	...
	public static void main(String args[]) {
		...
		try (JavaSparkContext javaSparkContext = new JavaSparkContext(conf)) {
			...
			JavaRDD<Tuple2<Integer, Employee>> empsNamesInUpperCasePairRdd = empsPairRdd.map(tuple -> {
				Employee emp = tuple._2;
				String nameInUpperCase = emp.getFirstName().toUpperCase();

				return new Tuple2<>(Integer.valueOf(tuple._1), new Employee(tuple._1, nameInUpperCase));

			});

			empsNamesInUpperCasePairRdd.collect().forEach(tuple -> {
